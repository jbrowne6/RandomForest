...
void classRF(double *x, int *dimx, int *cl, int *ncl, int *cat, int *maxcat,
	...
	randomize_x =	(double *) S_alloc(mdim*nsample, sizeof(double));//holds changed x
	...
	if (stratify) {//NOT STRATIFIED, IGNORE FOR NOW.
	...
	} else { //THIS IS WHERE STRATIFIED ENDS
...
/////////////////////////////////////////////////////////////////////////////////
/*randx(x, mdim, nsample, .20);
  Rprintf("nsample is %d, and mdim is %d\n", nsample, mdim); 
//  Rprintf("x1 is %f, x2 is %f, x3 is %f\n", x[3], x[4], x[5]);
   for (int m = 0; m <  nsample; m++){
for (int k = 0; k < mdim; k++){
		Rprintf("%f ", x[m*mdim+k]);
}Rprintf("\n");
}*////////////////////////////////////////////////////////////////////////////
	if (*labelts) {
...
memcpy(randomize_x, x, mdim*nsample*sizeof(double));
int deleteMe =0;
idxByNnode = 0 ;
idxByNsample = 0;
		GetRNGstate();
...
//*///
	//	memcpy(x,randomize_x, mdim*nsample* sizeof(double));
randx(x, mdim, nsample, .20);
...
/*	idxByNnode = 0;
	...
	if (addClass) createClass(x, nsample0, nsample, mdim);//not called
		do {
...
memcpy(x,randomize_x, mdim*nsample* sizeof(double));//copy origional X back to x to test
