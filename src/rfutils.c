...
void randx(double *x, double *XA, const int mdim, const int nsample, const int mtry, const int rerf, int *AHold, const int jb){
	...
	int err_print = 0; //if 1 print. else don't print.
	double A[mdim*mtry];
	zeroDouble(XA, mtry*nsample);
	int target_used = mdim*(mtry-1);
//				int target_used = mdim;
	float percent_target = (float)target_used/(mdim*mtry);
	int num_used = 0;
	if (rerf == 20){
	...
	} else if (rerf == 3 || rerf == 4 || rerf == 5){
target_used = mdim*(mtry-1);
percent_target = (float)target_used/(mdim*mtry);
		while(num_used != target_used){
			num_used = 0;
			zeroDouble(A, mdim*mtry);
			for (int q = 0; q < mdim*mtry; q++)
			{
				if(unif_rand() < percent_target){
					num_used++;
					if(unif_rand()<.5){
						A[q]=-1;
					}else{
