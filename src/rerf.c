 ...
 *                             Re-written from the original main program in
 *                             Fortran.
 *                               Andy Liaw Feb. 7, 2002.
 *                                 Modifications to get the forest out Matt
 *                                 Wiener Feb. 26, 2002.
 *                                  *****************************************************************/
...
void RerRF (double *x, int *dimx, int *cl, int *ncl, int *cat, int *maxcat,
		...
		int *inbag)
{
	 ...
	 *  nvar:     number of predictors to use for each
	 split
	 *  ipi:      0=use class proportion as prob.; 1=use
	 supplied priors
	 *  pi:       double vector of class priors
	 *  nodesize: minimum node size: no node with fewer
	 than ndsize
	 *            cases will be split
	 *
	 *  Output:
	 ...
	 *  imprt:    matrix of variable importance
	 measures
	 *  impmat:   matrix of local variable
	 importance measures
	 *  prox:     matrix of proximity (if
	 iprox=1)
	 ******************************************************************/

	int nsample0, mdim, nclass, addClass,
		mtry, ntest, nsample, ndsize,
		mimp, nimp, near, nuse, noutall,
		nrightall, nrightimpall,
		keepInbag, nstrata;
	int jb, j, n, m, k, idxByNnode,
		idxByNsample, imp, localImp, iprox,
		oobprox, keepf, replace, stratify, trace,
		*nright, *nrightimp, *nout, *nclts,
		Ntree, rerf;//added rerf fo randomerForest

	int *out, *bestsplitnext, *bestsplit,
		*nodepop, *jin, *nodex,
		*nodexts, *nodestart, *ta, *ncase, *jerr,
		*varUsed,
		*jtr, *classFreq, *idmove, *jvr,
		*at, *a, *b, *mind, *nind, *jts,
		*oobpair;
	int **strata_idx, *strata_size, last,
		ktmp, nEmpty, ntry;

	double av = 0.0, delta = 0.0;

	double *tgini, *tx, *wl, *classpop,
		   *tclasscat, *tclasspop, *win, *tp, *wr,
		   *XA;//added randomize_x for randomerForest
	...
	wl = (double *) S_alloc (nclass, sizeof
			(double));
	wr = (double *) S_alloc (nclass, sizeof
			(double));
	classpop = (double *) S_alloc (nclass *
			*nrnodes, sizeof (double));
	tclasscat = (double *) S_alloc (nclass *
			MAX_CAT, sizeof (double));
	tclasspop = (double *) S_alloc (nclass,
			sizeof (double));
	tx = (double *) S_alloc (nsample, sizeof
			(double));
	win = (double *) S_alloc (nsample, sizeof
			(double));
	tp = (double *) S_alloc (nsample, sizeof
			(double));

	out = (int *) S_alloc (nsample, sizeof
			(int));
	bestsplitnext = (int *) S_alloc
		(*nrnodes, sizeof (int));
	bestsplit = (int *) S_alloc (*nrnodes,
			sizeof (int));
	nodepop = (int *) S_alloc (*nrnodes,
			sizeof (int));
	nodestart = (int *) S_alloc (*nrnodes,
			sizeof (int));
	jin = (int *) S_alloc (nsample, sizeof
			(int));
	nodex = (int *) S_alloc (nsample, sizeof
			(int));
	nodexts = (int *) S_alloc (ntest, sizeof
			(int));
	ta = (int *) S_alloc (nsample, sizeof
			(int));
	ncase = (int *) S_alloc (nsample, sizeof
			(int));
	jerr = (int *) S_alloc (nsample, sizeof
			(int));
	jtr = (int *) S_alloc (nsample, sizeof
			(int));
	jvr = (int *) S_alloc (nsample, sizeof
			(int));
	classFreq = (int *) S_alloc (nclass,
			sizeof (int));
	jts = (int *) S_alloc (ntest, sizeof
			(int));
	idmove = (int *) S_alloc (nsample, sizeof
			(int));

	at = (int *) S_alloc (mdim * nsample,
			sizeof (int));
	XA =(double *) S_alloc (mdim * nsample,
			sizeof (double));//added for randomerForest
	a = (int *) S_alloc (mdim * nsample,
			sizeof (int));
	b = (int *) S_alloc (mdim * nsample,
			sizeof (int));
	mind = (int *) S_alloc (mdim, sizeof
			(int));
	tgini = (double *) S_alloc (mdim, sizeof
			(double));
	varUsed = (int *) S_alloc (mdim, sizeof
			(int));
	nrightimp = (int *) S_alloc (nclass,
			sizeof (int));
	nout = (int *) S_alloc (nclass, sizeof
			(int));
	if (oobprox)
	{
		oobpair = (int *) S_alloc (near * near,
				sizeof (int));
	}
	...
	*/
	...
	normClassWt (cl, nsample, nclass, *ipi,
			classwt, classFreq);

	if (stratify)
	{
		/* Count number of strata and frequency
		   of each stratum. */
		...
		for (n = 0; n < nsample0; ++n)
			if (strata[n] > nstrata)
				nstrata = strata[n];
		/* Create the array of pointers, each
		   pointing to a vector
		   of indices of where data of each
		   stratum is. */
	...
	if (iprox)
	{
		zeroDouble (prox, nsample0 *
				nsample0);
		if (*testdat)
			zeroDouble (proxts, ntest * (ntest +
						nsample0));
	}

	R_CheckUserInterrupt ();

	/* Starting the main loop over number
	   of trees. */
	GetRNGstate ();
	if (trace <= Ntree)
	{
	...
	if (rerf == 0){  //if rerf = 0 then just do normal random forest.
		makeA (x, mdim, nsample, cat, at,
				b);//moved for randomerForest
	}
	idxByNnode = 0;
	idxByNsample = 0;
	...
	{
		if (rerf > 0){
			if (rerf == 3 || rerf == 6 || rerf ==
					20)//just used to test when mdim == mtry.
			{
				randx(x, XA, mdim_hold, nsample,
						*dSize, rerf, AHold, jb);//added for randomerForest
		...
		/* Do we need to simulate data for the
		   second class? */
		if (addClass)
		...
		do
		{
			...
			/* TODO: Put all sampling code into a
			   function. */
			...
			{/* stratified sampling */
				...
				{/* stratified sampling w/o
					...
					for (n = 0; n < nstrata; ++n)
					{
						...
						for (j = 0; j < sampsize[n]; ++j)
						{
							ktmp = (int) (unif_rand () * (last +
										1));
							k = strata_idx[n][ktmp];
							swapInt (strata_idx[n][last],
									strata_idx[n][ktmp]);
							last--;
							tclasspop[cl[k] - 1] += classwt[cl[k]
								- 1];
							win[k] += classwt[cl[k] - 1];
							jin[k] += 1;
						}
			...
			{/* unstratified sampling */
				...
				do
				{
					...
					/* check if any class is missing in
					   the sample */
					for (n = 0; n < nclass; ++n)
					{
						if (tclasspop[n] == 0.0)
				...
				while (nclass - nEmpty < 2 && ntry <=
						30);
				/* If there are still fewer than two
				   classes in the data, throw an error.
				   */
				if (nclass - nEmpty < 2)
					error("Still have fewer than two classes in the in-bag sample after 30 attempts.");
			}

			/* If need to keep indices of inbag
			   data, do that here. */
			if (keepInbag)
			{
				for (n = 0; n < nsample0; ++n)
			...
			memcpy (a, at, sizeof (int) * mdim *
					nsample);
			modA (a, &nuse, nsample, mdim, cat,
					*maxcat, ncase, jin);

			F77_CALL (buildtree) (a, b, cl, cat,
					maxcat, &mdim, &nsample,
					&nclass,
					treemap + 2 * idxByNnode,
					bestvar + idxByNnode, bestsplit,
					bestsplitnext, tgini, nodestatus +
					idxByNnode,
					nodepop, nodestart, classpop,
					tclasspop,
					tclasscat, ta, nrnodes, idmove,
					&ndsize,
					ncase, &mtry, varUsed, nodeclass +
					idxByNnode,
		...
		Xtranslate (XA, mdim, *nrnodes,
				nsample, bestvar + idxByNnode,
				bestsplit, bestsplitnext, xbestsplit +
				idxByNnode,
				nodestatus + idxByNnode, cat,
				ndbigtree[jb]);

		/*  Get test set error */
		if (*testdat)
		{
			predictClassTree (xts, ntest, mdim,
					treemap + 2 * idxByNnode,
					nodestatus + idxByNnode, xbestsplit +
					idxByNnode,
					bestvar + idxByNnode,
					nodeclass + idxByNnode, ndbigtree[jb],
					cat, nclass, jts, nodexts, *maxcat);
			TestSetError (countts, jts, clts,
					outclts, ntest, nclass, jb + 1,
					errts + jb * (nclass + 1), *labelts,
					nclts, cut);
		}

		/*  Get out-of-bag predictions and
			errors. */
		...
		for (n = 0; n < nsample; ++n)
		{
			if (jin[n] == 0)
			{
				...
				*/
				out[n]++;
				/* count number of OOB cases in the
				   current iteration.
				   nout[n] is the number of OOB cases
				   for the n-th class.
				   noutall is the number of OOB
				   cases overall. */
				nout[cl[n] - 1]++;
				noutall++;
			}
		...
		if ((jb + 1) % trace == 0)
		{
			Rprintf ("%5i: %6.2f%%", jb + 1,
					100.0 * errtr[jb * (nclass +
						1)]);
			for (n = 1; n <= nclass; ++n)
			{
				Rprintf ("%6.2f%%", 100.0 *
		...
		if (iprox)
		{
			computeProximity (prox, oobprox,
					nodex, jin, oobpair, near);
			/* proximity for test data */
			if (*testdat)
			{
				computeProximity (proxts, 0,
						nodexts, jin, oobpair, ntest);
				/* Compute proximity between
				   testset and training set. */
				for (n = 0; n < ntest; ++n)
				{
					for (k = 0; k < near; ++k)
					{
						if (nodexts[n] == nodex[k])
							proxts[n + ntest * (k + ntest)]
								+= 1.0;
					}
				}
			}
		...
		if (imp)
		{
			...
			/* Count the number of correct
			   ...
			   among the OOB samples, by
			   class. */
			zeroInt (nright, nclass);
			for (n = 0; n < nsample; ++n)
			{
				/* out-of-bag and predicted
correctly: */
				if (jin[n] == 0 && jtr[n] ==
						cl[n])
				{
				...
				if (varUsed[m])
				{
					...
					*/
					...
					predictClassTree (XA,
							...
							ndbigtree[jb],
							cat, nclass, jvr, nodex,
							*maxcat);
					/* Count how often correct
		...
		//mdim = mdim_hold;
		//reset mdim every iteration so that randx can recreate XA every time.
	}
	PutRNGstate ();
