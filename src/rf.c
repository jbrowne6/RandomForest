...
void
classRF (double *x, int *dimx, int *cl, int *ncl, int *cat, int *maxcat,
	 ...
	 int *inbag)
{
  ...
  double *tgini, *tx, *wl, *classpop, *tclasscat, *tclasspop, *win, *tp, *wr, *randomize_x;	//added randomize_x for randomerForest
  ...
  randomize_x =(double *) S_alloc (mdim * nsample, sizeof (double));	//added for randomerForest
...
if (rerf > 0){
  ...
  makeA (x, mdim, nsample, cat, at, b);	//moved for randomerForest
      ...
      do
	{
	  ...
	  memcpy(x, randomize_x, mdim*nsample*sizeof(double));//added for randomerForest.
      ...
      if (imp)
	{
	  ...
	  for (m = 0; m < mdim; ++m)
	      ...
	      if (varUsed[m])
		{
		  ...
		  predictClassTree (x, nsample, mdim,
				    treemap + 2 * idxByNnode,
...
void
classForest (int *mdim, int *ntest, int *nclass, int *maxcat,
	     ...
	     int *keepPred, int *prox, double *proxMat, int *nodes)
{
  ...
  for (j = 0; j < *ntree; ++j)
      ...
      for (n = 0; n < *ntest; ++n)
	{
      ...
      if (*prox)
	computeProximity (proxMat, 0, node + offset2, junk, junk, *ntest);
      idxNodes += *nrnodes;
      if (*keepPred)
	offset1 += *ntest;
      if (*nodes)
...
*/
void
oob (int nsample, int nclass, int *cl, int *jtr, int *jerr,
     int *counttr, int *out, double *errtr, int *jest, double *cutoff)
{
...
void
TestSetError (double *countts, int *jts, int *clts, int *jet, int ntest,
	      int nclass, int nvote, double *errts,
	      int labelts, int *nclts, double *cutoff)
{
  int j, n, ntie;
